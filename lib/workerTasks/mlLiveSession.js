...
function fetchChannels () {
	return fetch(wpApiBaseUrl + '/notify/channels').then((res) => {
		if (res.ok) {
			return res.json();
		} else {
		...
		.then((results) => {
			if (results) {
				results.forEach((session) => {
					const wpApi = new WpApi(`${process.env.WP_URL}/${session.url}`);

					wpApi.status().then((json) => {
