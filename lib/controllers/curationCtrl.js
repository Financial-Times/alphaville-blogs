...
const curationOptions = [
	...
	},
	{
		...
		applicable: article => article.authors.length && article.authors[0].headshotUrl
	},
	{
		value: 'imagelead',
		label: 'Image lead',
...
exports.index = (req, res, next) => {
	...
	articleService.getArticles({
		...
		.then(response => {
			...
			const heroIndex = response.items.findIndex(function(item) {
				return item.isHero;
			});

			if (heroIndex) {
				const hero = response.items.splice(heroIndex, 1);

				if (hero && hero.length) {
					response.items.unshift(hero[0]);
				}
			}

			response.items.forEach((article, index) => {
