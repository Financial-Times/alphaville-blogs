...
exports.index = function (req, res, next) {
		...
		articleService.getMarketsliveArticles({
			...
			.then(response => {
				...
				res.render('ml-index', {
					...
					mlApiUrl: 'http://local.ft.com:3030',//process.env.ML_API_URL,
	...
	liveSessionService.latest().then((session) => {
		if (session && req.query.force !== 'home' && !req.query['invitation-token']) {
...
exports.byVanity = function (req, res, next) {
	return articleService.getArticle(`/marketslive/${req.params[0]}`).then((article) => {
		...
		function getAnnotations (type) {
			let results = article.annotations.filter(item => item.type === type && item.predicate === 'http://www.ft.com/ontology/classification/isPrimarilyClassifiedBy');
			if (!results.length) {
				results = article.annotations.filter(item => item.type === type && item.predicate === 'http://www.ft.com/ontology/classification/isClassifiedBy');
			}
		...
		if (article.isMarketsLive === true) {
			if (article.isLive === true || article.isComingSoon === true) {
				...
				const renderData = {
					...
					apiUrl: `${process.env.ML_API_URL.replace('192.168.99.100', 'local.ft.com')}/${req.params[0]}`,
				...
				if (req.query.ajax) {
					...
					res.render('ml-transcript', {
						...
						article: article,
						articleId: article.id,
						primaryTheme : getAnnotations('SECTION')[0].prefLabel,
