	...
	response.hits.hits.forEach(function(obj, index) {
		...
		if (obj.isMarketsLive === true) {
			...
			if (filterMetadataBy({ prefLabel: 'First FT' }).length > 0) {
				...
				} else if (obj._source.mainImage && !isAuthorLeadWithImageSelected && authors.length > 0 && authorHeadshot && obj._source.title.indexOf('Alphachat:') === -1 && obj._source.title.indexOf('Thought for the weekend') === -1) {
