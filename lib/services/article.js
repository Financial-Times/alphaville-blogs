...
exports.getArticles = function (options) {
	let url = `${esServiceUrl}/articles?${qs.stringify(options)}`;
	if (options && options.purge === true) {
...
exports.getMarketsliveArticles = function (options) {
	let url = `${esServiceUrl}/marketslive?${qs.stringify(options)}`;
	if (options && options.purge === true) {
		url += '&_=' + new Date().getTime();
	}
	return getAvEsServiceContent(url);
...
exports.getHotArticles = function (options) {
	let url = `${esServiceUrl}/hotarticles?${qs.stringify(options)}`;
	if (options && options.purge === true) {
		url += '&_=' + new Date().getTime();
	}
	return getAvEsServiceContent(url);
