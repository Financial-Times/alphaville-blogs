		...
		.then((curation) => {
			...
			response.hits.hits.forEach((article) => {
				...
				if (article.isMarketsLive === true) {
					...
					// if (article._source.title.indexOf('Alphachat') !== -1 || article._source.title.indexOf('Podcast') !== -1) {
					// 	article._source.primaryTheme = 'Alphachat';
					...
					// } else if (article._source.seriesArticles) {
					// 	article._source.primaryTheme = article._source.seriesArticles.series.prefLabel;
...
exports.getArticle = function(vanityOrUuid) {
	let url = `${esServiceUrl}/${vanityOrUuid}`;
	return getAvEsServiceContent(url);
};

exports.getArticles = function (q, limit, offset) {
	let url = `${esServiceUrl}/articles?${qs.stringify({q, limit, offset})}`;
	return getAvEsServiceContent(url);
};

exports.getMarketsliveArticles = function (limit, offset) {
	let url = `${esServiceUrl}/marketslive?${qs.stringify({limit, offset})}`;
	return getAvEsServiceContent(url);
};
