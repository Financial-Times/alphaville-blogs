...
const getArticles = (options, endpoint = 'articles') => {
	options = options || {};
...
exports.getArticles2 = getArticles;



exports.getArticles = function (options) {
	options = options || {};
	let url = `${esServiceUrl}/articles?${qs.stringify(_.omit(options, ['noCache']))}`;
	if (options && options.noCache === true) {
		url += '&_=' + new Date().getTime();
	}
	return getAvEsServiceContent(url);
...
exports.getArticlesByTopic = function (options) {
	options = options || {};
	let url = `${esServiceUrl}/topic?${qs.stringify(_.omit(options, ['noCache']))}`;
	if (options && options.noCache === true) {
		url += '&_=' + new Date().getTime();
	}
	return getAvEsServiceContent(url);
...
exports.getArticlesByAuthor = function (options) {
	options = options || {};
	let url = `${esServiceUrl}/author?${qs.stringify(_.omit(options, ['noCache']))}`;
	if (options && options.noCache === true) {
		url += '&_=' + new Date().getTime();
	}
	return getAvEsServiceContent(url);
...
exports.getMarketsliveArticles = function (options) {
	options = options || {};

	let url = `${esServiceUrl}/marketslive?${qs.stringify(_.omit(options, ['noCache']))}`;
	if (options && options.noCache === true) {
		url += '&_=' + new Date().getTime();
	}
	return getAvEsServiceContent(url);
...
exports.getHotArticles = function (options) {
	options = options || {};
	let url = `${esServiceUrl}/hotarticles?${qs.stringify(_.omit(options, ['noCache']))}`;
	if (options && options.noCache === true) {
		url += '&_=' + new Date().getTime();
	}
	return getAvEsServiceContent(url);
...
exports.getMostReadArticles = function (options) {
	options = options || {};
	let url = `${esServiceUrl}/most-read?${qs.stringify(_.omit(options, ['noCache']))}`;
	if (options && options.noCache === true) {
		url += '&_=' + new Date().getTime();
	}
	return getAvEsServiceContent(url);
...
exports.getMostCommentedArticles = function (options) {
	options = options || {};
	let url = `${esServiceUrl}/most-commented?${qs.stringify(_.omit(options, ['noCache']))}`;
	if (options && options.noCache === true) {
		url += '&_=' + new Date().getTime();
	}
	return getAvEsServiceContent(url);
...
exports.getSeriesArticles = function(options) {
	options = options || {};
	const id = options.seriesId || '';
	let url = `${esServiceUrl}/series/${id}?${qs.stringify(_.omit(options, ['noCache', 'seriesId']))}`;
	if (options && options.noCache === true) {
		url += '&_=' + new Date().getTime();
	}
	return getAvEsServiceContent(url);
