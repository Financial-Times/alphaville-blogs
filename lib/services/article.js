...
exports.categorization = function (response, firstPage) {
	...
	response.hits.hits.forEach((obj, index) => {
		...
		if (obj._source.av2WebUrl.startsWith('/')) {
			obj.withInlineRead = true;
		}
		...
		if (obj.isMarketsLive === true) {
			...
			if (obj.isLive === true) {
				delete obj.withInlineRead;
			}
		} else {
			if (filterMetadataBy({ prefLabel: 'First FT' }).length > 0) {
				obj.isFirstFT = true;
				obj._source.primaryTheme = 'First FT';
			} else {
				const sections = filterMetadataBy({ primary: 'section', taxonomy: 'sections' });
				if (sections && sections.length) {
					obj._source.primaryTheme = sections[0].prefLabel;
				} else {
					obj._source.primaryTheme = "";
				}
				...
				if (firstPage) {
					if (!isHeroSelected && obj._source.title.indexOf('FT Opening Quote') === -1) {
						if (new Date().getTime() - new Date(obj._source.publishedDate).getTime() < 48 * 60 * 60 * 1000) {
							obj.isHero = true;
							delete obj.isBlog;
							isHeroSelected = true;
						}
					...
					if (obj.isBlog) {
						if (obj._source.authors.length && authorsOpinion.indexOf(obj._source.authors[0].name) === -1 && obj._source.authors[0].headshotUrl) {
							obj.isOpinion = true;
							delete obj.isBlog;
							authorsOpinion.push(obj._source.authors[0].name);
						}
					}
				}
			}
...
exports.getRecentPosts = function (count) {
